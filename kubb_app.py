...
# --- 8. LIVE APP PHASE ---
    ...
    # --- 9. STANDINGS CALCULATION (FIXED LOSSES) ---
    all_teams = pd.unique(prelims[['Team A', 'Team B']].values.ravel())
    all_teams = [t for t in all_teams if t not in ["BYE", "nan", "None", ""]]
    standings_list = []
    for t in all_teams:
        # Wins: Matches where this team is the 'Winner'
        wins = len(prelims[prelims['Winner'] == t])
        
        # Losses: Matches where this team played, a winner was declared, but it wasn't them
        losses = len(prelims[((prelims['Team A'] == t) | (prelims['Team B'] == t)) & 
                            (prelims['Winner'] != "None") & 
                            (prelims['Winner'] != t)])
        
        # Strength of Schedule (SoS): Sum of opponents' wins
        ...
        standings_list.append({"Team": t, "Wins": wins, "Losses": losses, "SoS": sos, "GP": wins + losses})
